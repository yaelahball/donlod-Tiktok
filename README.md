...
2. Resolusi berlapis (fallback otomatis, tahan WAF). Urutan:
   - Halaman video desktop (universal data) — **kandidat paling lengkap** (bitrate/
     resolusi/codec) — dengan **pemecah WAF proof-of-work** (Slardar challenge
     dihitung murni via `hashlib`, cookie `_wafchallengeid`)
   - Web API `https://www.tiktok.com/api/item/detail/?aid=1988&itemId=...`
...
4. Mode `--user`: resolve `secUid` (seed video → profil HTML tahan-WAF → `user/detail`
   API → video pertama dari profil embed), lalu paginate
   `https://www.tiktok.com/api/creator/item_list/`

> Catatan: endpoint `item/detail` bisa balas kosong dari beberapa jaringan; halaman
> profil/produk yang diproteksi Slardar ditangani pemecah WAF. `creator/item_list`
> bekerja normal **asalkan `secUid` lengkap** (76 char) diperoleh. Cookie `ms_token`
> diambil dari browser login kamu.
